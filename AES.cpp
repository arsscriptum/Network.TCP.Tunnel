...
LONG AESInitKey(const unsigned char *key, ULONG keylen, AesKey_t *skey);
void AESEncryptBlock(const unsigned char *pt, unsigned char *ct, AesKey_t *skey);
void AESDecryptBlock(const unsigned char *ct, unsigned char *pt, AesKey_t *skey);
LONG AESInit(AesContext_t *ctx, const unsigned char *key, ULONG keylen);
LONG AESInit(AesContext_t *ctx, unsigned char *IV, const unsigned char *key, ULONG keylen);
LONG AESEncrypt(AesContext_t *ctx, unsigned char *pt, unsigned char *ct, ULONG len);
LONG AESDecrypt(AesContext_t *ctx, unsigned char *ct, unsigned char *pt, ULONG len);

void HexToBytes(const char *hex, int length, BYTE *bytes)
{
	char hexchars[256] = {};
	length -= length % 2;
	for (int i=0; i < length; i += 2)
	{
		*bytes++ = (hexchars[(BYTE)hex[i]] << 4) + hexchars[(BYTE)hex[i + 1]];
	}
}
...
void BytesToHex(const BYTE *bytes, int length, char *hex)
{
	...
	{
		*hex++ = hexchars[(*bytes & 0xf0) >> 4];
		*hex++ = hexchars[*bytes & 0x0f];
		bytes++;
	}
}
...
{
	AesContext_t ctx={0};
	if(ivlength != sizeof(ctx.IV))
	{
		return false;
	}

	int enclength = length - (length % ivlength);

	if (enclength > 0)
	{
		if (AESInit(&ctx, iv, key, keylength) == -1)
		{
			return false;
		}

		if (AESEncrypt(&ctx, data, ctext, enclength) == -1)
		{
			return false;
		}

		memcpy(iv, ctx.IV, ivlength);
	}

	if(enclength < length)
	{
		// CFB for any leftover data where CFB data encryption must be in single pass (once only)
		unsigned char * input = data + enclength;
		unsigned char * output = ctext + enclength;
		int len = length - enclength, n = 0;
		unsigned char * ivcfb = (unsigned char *)_alloca(ivlength);
		memcpy(ivcfb, iv, ivlength);
		while(len--)
		{
			if(n == 0)
			{
				AESInit(&ctx, key, keylength);
				AESEncrypt(&ctx, ivcfb, ivcfb, ivlength);
			}

			unsigned char c = *input++;
			*output++ = (unsigned char)(c ^ ivcfb[n]);
			ivcfb[n] = (unsigned char)c;
			n = (n + 1) % ivlength;
		}
	}
...
{
	int keylength = (key ? strlen(key) : 0) / 2;
	unsigned char * binkey = (unsigned char *)_alloca(keylength);
	HexToBytes(key, 2 * keylength, binkey);

	return AESCBCEncrypt(binkey, keylength, iv, ivlength, data, ctext, length);
}
...
bool AESCBCDecrypt(const unsigned char * key, int keylength, unsigned char * iv, int ivlength, unsigned char * ctext, unsigned char * data, int length)
{
	...
	int declength = length - (length % ivlength);
	...
	if (declength > 0)
	{
		...
		if(AESDecrypt(&ctx, ctext, data, declength) == -1)
		{
			return false;
		}
	...
	if(declength < length)
	{
		unsigned char * input = ctext + declength;
		unsigned char * output = data + declength;
		int len = length - declength, n = 0;
		unsigned char * ivcfb = (unsigned char *)_alloca(ivlength);
		memcpy(ivcfb, iv, ivlength);
		while(len--)
		{
...
bool AESCBCDecrypt(const char * key, unsigned char * iv, int ivlength, unsigned char * ctext, unsigned char * data, int length)
{
	...
	return AESCBCDecrypt(binkey, keylength, iv, ivlength, ctext, data, length);
}
 ...
 */
LONG AESInit(AesContext_t *ctx, unsigned char *IV,
				  const unsigned char *key, ULONG keylen)
{
	...
	memset(ctx, 0x0, sizeof(AesContext_t));
/*
	setup cipher
 */
	if ((err = AESInitKey(key, keylen, &ctx->key)) != ERROR_SUCCESS) {
		return err;
	}
/*
	copy IV
 */
	ctx->blocklen = AES_BLOCK_SIZE;
...
LONG AESEncrypt(AesContext_t *ctx, unsigned char *pt,
					 unsigned char *ct, ULONG len)
{
	LONG			x;
	ULONG			i;
...
/*
	is blocklen valid?
 */
	if (ctx->blocklen < 0 || (ctx->blocklen > 
			(LONG)sizeof(ctx->IV))) {
		return -1;
	}

	for (i = 0; i < len; i += ctx->blocklen) {
/*
...
/*
		encrypt
 */
		AESEncryptBlock(tmp, ct, &ctx->key);
...
LONG AESDecrypt(AesContext_t *ctx, unsigned char *ct,
					 unsigned char *pt, ULONG len)
{
	LONG			x;
	ULONG			i;
 ...
 */
		AESDecryptBlock(ct, tmp, &ctx->key);
/*
...
LONG AESInitKey(const unsigned char *key, ULONG keylen, AesKey_t *skey)
{
	...
	memset(skey, 0x0, sizeof(AesKey_t));
...
void AESEncryptBlock(const unsigned char *pt, unsigned char *ct,
				AesKey_t *skey)
{
...
void AESDecryptBlock(const unsigned char *ct, unsigned char *pt,
				AesKey_t *skey)
{
	ULONG		s0, s1, s2, s3, t0, t1, t2, t3, *rk;
	LONG		Nr, r;
